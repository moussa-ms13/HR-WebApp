  ...
  useEffect(() => {
    ...
    const initializeModal = async () => {
      ...
      if (isEdit && empId) {
        ...
        setProfileImagePreview(employee.ProfileImagePath ? `http://localhost:5000${employee.ProfileImagePath}` : null);
        ...
        try {
          ...
          if (isMounted && fullData) {
            ...
            if (fullData.ProfileImagePath) {
              setProfileImagePreview(`http://localhost:5000${fullData.ProfileImagePath}`);
            }
          }
        } catch (err) {
          ...
          <form id="employeeForm" onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">المنصب العالي (Assigned Position)</label>

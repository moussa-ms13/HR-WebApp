 ...
 */
router.post("/", hasPermission(PERMISSIONS.EDIT), upload.any(), async (req, res) => {
 ...
 */
router.post("/batch", hasPermission(PERMISSIONS.EDIT), upload.any(), async (req, res) => {
  const files = req.files || [];
  if (files.length === 0) {
    return res.status(400).json({ success: false, message: "لم يتم تحديد أي ملفات للرفع." });
  }
 ...
 */
router.post("/bulk-delete", express.json(), hasPermission(PERMISSIONS.DELETE), async (req, res) => {
 ...
 * Delete a document
 */
router.delete("/:fileId", hasPermission(PERMISSIONS.DELETE), async (req, res) => {
 ...
 */
router.put("/:fileId", hasPermission(PERMISSIONS.EDIT), upload.single("file"), async (req, res) => {
